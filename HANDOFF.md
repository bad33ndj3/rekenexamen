...
8. ✅ Klaar (`b08255e` + `a8e3d24`, browser 6/6 groen op 390px): rekenmachine-tool — inschuifpaneel van rechts (`#calc-panel`, max 340px), knop in sticky actiebalk van ALLE vraagweergaven (leren/herstel/nulmeting/proefexamen; gebruikerskeuze), rekenvolgorde (3+4×2=11), komma-notatie, %, ±, C, ⌫, toetsenbordsteun, nette `Delen door nul kan niet`, `role=dialog` + `aria-live`, `prefers-reduced-motion`, raakt attempts/mastery/timer niet. Bekend gedrag: open paneel bedekt op 390px de antwoordknoppen (eerst sluiten, dan antwoorden).
9. ✅ Klaar (`6cb17f6`): backendloos bewaren/delen in `#/begeleider` — `Bewaar kopie` (download `rekenen-voortgang-YYYYMMDD.json` naar Bestanden/Downloads), `Stuur naar begeleider` (deelmenu telefoon via Web Share met files; fallback TXT → klembord + instructie; annuleren is stil), import heet nu `Zet terug`, plus eenmalig `navigator.storage.persist()` als best-effort tegen wissen (iOS kan storage na ~7 dagen niet-gebruiken wissen).
10. ✅ Public (`bad33ndj3/rekenexamen`, MIT): Pages-workflow (Actions-bron, deploy-pages v4), Taskfile getrackt, persoonlijke paden opgeschoond, uitgevers-PDF uit repo + historie verwijderd (lokaal bewaard, genegeerd). Live op https://bad33ndj3.github.io/rekenexamen/ (eerste groene deploy 2026-09-16 ~22:17).
